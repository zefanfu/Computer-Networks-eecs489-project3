...
int main(int argc, char *argv[]) {
	...
	while (true) {
		...
		if ((numbytes = recvfrom(sockfd, dbuf, PACKET_SIZE , 0,(struct sockaddr *)&their_addr, &addr_len)) == -1) {
			perror("recvfrom");
			exit(1);
		}
		...
		if (dheader.type == 0) { // START
            ...
            if (dheader.seqNum >= expSeqNum) { 
				...
				// //set bool
				// window[dheader.seqNum - expSeqNum]->hasAck=1;				
				// store data
